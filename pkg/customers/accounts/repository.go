...
func NewRepo(db *sql.DB) *sqlAccountRepository {
...
func (r *sqlAccountRepository) getCustomerAccounts(customerID string) ([]*client.Account, error) {
	...
	for i := range accountIDs {
		acct, err := r.getCustomerAccount(customerID, accountIDs[i])
		if err != nil {
			return nil, fmt.Errorf("problem reading accountID=%s error=%v", accountIDs[i], err)
		}
		if acct != nil {
			out = append(out, acct)
		}

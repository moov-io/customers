...
func TestCustomerSSNRepository(t *testing.T) {
	...
	// SQLite tests
	check(t, createTestCustomerSSNRepository(t))

	// MySQL tests
	mysqlDB := database.CreateTestMySQLDB(t)
	defer mysqlDB.Close()
	check(t, &sqlCustomerSSNRepository{mysqlDB.DB})
}

func createTestCustomerSSNRepository(t *testing.T) *sqlCustomerSSNRepository {
	t.Helper()
	...
	return &sqlCustomerSSNRepository{db}
}

...
func TestCustomers__GetCustomer(t *testing.T) {
	...
	NewCustomersController(logger, createTestOFACSearcher(t, nil, nil), repo, testCustomerSSNStorage(t)).AppendRoutes(router)
...
func TestCustomerRepository__createCustomer(t *testing.T) {
	...
	// SQLite tests
	check(t, createTestCustomerRepository(t))

	// MySQL tests
	mysqlDB := database.CreateTestMySQLDB(t)
	defer mysqlDB.Close()
	check(t, &sqlCustomerRepository{mysqlDB.DB})
}
...
func TestCustomers__createCustomer(t *testing.T) {
	...
	router := mux.NewRouter()
	NewCustomersController(logger, createTestOFACSearcher(t, nil, nil), repo, customerSSNStorage).AppendRoutes(router)
...
func TestCustomers__replaceCustomerMetadata(t *testing.T) {
	...
	NewCustomersController(logger, createTestOFACSearcher(t, nil, nil), repo2, testCustomerSSNStorage(t)).AppendRoutes(router2)
	router2.Use(testMiddleware.Handler)

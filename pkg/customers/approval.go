...
func addApprovalRoutes(logger logging.Logger, svc *admin.Server, repo customerRepository) {
...
func updateCustomerStatus(logger logging.Logger, repo customerRepository) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if customerID == "" {
			moovhttp.Problem(w, errors.New("missing customerID"))
			return
		}
...
func updateCustomerAddress(logger logging.Logger, repo customerRepository) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method != "PUT" {
			moovhttp.Problem(w, fmt.Errorf("unsupported HTTP verb %s", r.Method))
			return
		...
		if addressId == "" {
			moovhttp.Problem(w, errors.New("missing addressID"))
			return
		}
		...
		if err := repo.updateCustomerAddress(customerID, addressId, req.Type, req.Validated); err != nil {
			logger.Log(fmt.Sprintf("error updating customer=%s address=%s: %v", customerID, addressId, err))
			moovhttp.Problem(w, err)
			return
		}
		respondWithCustomer(logger, w, customerID, repo)

...
func (c *moovWatchmanClient) Search(ctx context.Context, name string, requestID string) (*watchman.OfacSdn, error) {
	...
	if len(search.SDNs) == 0 && len(search.AltNames) > 0 {
		...
		c.logger.WithKeyValue("watchman", fmt.Sprintf("Found AltName=%s,SDN=%s with no higher matched SDNs", alt.AlternateID, alt.EntityID), "requestID", requestID)
	...
	if len(search.SDNs) > 0 && len(search.AltNames) > 0 && (search.AltNames[0].Match > 0.1) && search.AltNames[0].Match > search.SDNs[0].Match {
		alt := search.AltNames[0]
		c.logger.WithKeyValue("watchman", fmt.Sprintf("AltName=%s,SDN=%s had higher match than SDN=%s", alt.AlternateID, alt.EntityID, search.SDNs[0].EntityID), "requestID", requestID)
...
func NewClient(logger log.Logger, endpoint string, debug bool) Client {
	...
	logger.WithKeyValue("watchman", fmt.Sprintf("using %s for Watchman address", conf.BasePath))

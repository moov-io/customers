...
func TestCustomers__GetCustomer(t *testing.T) {
	...
	addCustomerRoutes(log.NewNopLogger(), router, repo, testCustomerSSNStorage(t), createTestOFACSearcher(t, nil, nil))
...
func TestCustomers__createCustomer(t *testing.T) {
	...
	router := mux.NewRouter()
	addCustomerRoutes(log.NewNopLogger(), router, repo, customerSSNStorage, createTestOFACSearcher(t, nil, nil))
	router.ServeHTTP(w, req)
	w.Flush()
...
func TestCustomers__replaceCustomerMetadata(t *testing.T) {
	...
	addCustomerRoutes(log.NewNopLogger(), router2, repo2, testCustomerSSNStorage(t), createTestOFACSearcher(t, nil, nil))
	router.ServeHTTP(w, req)
	w.Flush()
...
func TestCustomers__BadReq(t *testing.T) {
	...
	router := mux.NewRouter()
	addCustomerRoutes(log.NewNopLogger(), router, nil, customerSSNStorage, createTestOFACSearcher(t, nil, nil))
	router.ServeHTTP(w, req)
	w.Flush()

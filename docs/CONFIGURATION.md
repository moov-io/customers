...
- `DOCUMENTS_SECRET_PROVIDER`: Determines which environment variables are used to initialize persistant document storage. Defaults to [local filesystem](#local-filesystem).
- `SSN_SECRET_PROVIDER`: Determines which environment variables are used to initialize SSN storage persistence. Defaults to [local filesystem](#local-filesystem).
